...
* 개발 중(next dev)에는 Fast Refresh(빠른 새로 고침)으로 CSS 업데이트가 즉시 적용됨
* 프로덕션 환경(next build)에서는 모든 CSS 파일이 자동으로 여러 개의 압축 및 코드 분할된 `.css` 파일로 연결되어 각 경로에 최소한의 CSS만 로드 되도록 보장
* 프로덕션 환경에서는 자바스크립트가 비활성화된 상태에서도 CSS가 로드되지만 개발 환경에서는 Fast Refresh를 위해 자바스크립트가 필요
* 개발 환경에서는 CSS 순서가 다르게 동작할 수 있기 때문에 최종 CSS 순서를 확인하려면 항상 빌드를 확인

###  이미지 최적화

* The Next.js `<Image>` 컴포넌트는 HTML `<img>` 요소를 확장하여 제공
* 크기 최적화: 각 기기에 맞게 자동으로 적절한 크기의 이미지를 제공하며 WebP와 같은 최신 이미지 형식을 사용할 수 있음
* 시각적 안정성: 이미지 로딩 시 레이아웃 이동을 자동으로 방지
* 더 빠른 페이지 로드: 기본 브라우저 지연 로딩을 사용하여 뷰포트에 들어올 때만 이미지를 로드하며, 선택적 흐리게 표시되는 placeholder를 제공
* 자산(Asset) 유연성: 원격 서버에 저장된 이미지도 원하는 대로 이미지 크기를 조절할 수 있음

### 로컬 이미지

* 정적 파일(이미지, 폰트 등)은 루트 디렉터리의 `public` 디렉터리에 저장할 수 있음
* `public` 디렉터리 내의 파일은 기본 URL(`/`)을 시작으로 코드에서 참조할 수 있음
...
### Eject to a Custom Server (사용자 지정 서버로 이젝트)

* Next.js가 기본적으로 제공하는 서버 동작 방식을 버리고 직접 Node.js 서버를 만들 Next.js를 얹어서 사용하는 것을 의미
* CRA(Create React App)에서 시작된 개념
* 기본적으로 Next.js는 자체 서버가 있고 이 서버는 라우팅, 정적 파일 서빙, 캐싱, 빌드 최적화 등을 자동으로 처리해 줌
* 하지만 특수한 기능이 필요하면 이 기본 서버를 버리고(eject) 직접 Node.js 서버를 만드는 것

### 왜 eject(커스텀 서버)가 필요할까?

* 1. Next.js 기본 서버로는 할 수 없는 서버 기능이 필요할 때
* Next.js의 라우팅/서버 구조를 내가 직접 다루고 싶은 경우

### Docker

* Next.js는 Docker 컨테이너를 지원하는 모든 제공업체에 배포할 수 있음
* 여기에는 Kubernetes와 같은 컨테이너 오케스트레이터나 Docker를 실행하는 클라우드 제공업체가 포함됨
* Docker 배포는 모든 Next.js 기능을 지원

### 정적 내보내기(Static export)

* Next.js는 정적 사이트 또는 SPA(Single-Page Application)으로 시작할 수 있으며 이후 서버가 필요한 기능을 사용하기 위해 선택적으로 업그레이드할 수 있음
* Next.js는 정적 내보내기를 지원하므로 HTML/CSS/JS 정적 자산을 제공할 수 있는 모든 웹 서버에 배포 및 호스팅할 수 있음
* 이런 호스팅에는 AWS S3, Nginx 또는 Apache와 같은 도구가 포함
* 정적 내보내기로 실행하는 경우 서버가 필요한 Next.js 기능은 지원되지 않음
